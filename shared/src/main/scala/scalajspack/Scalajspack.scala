  ...
  def serialize(options: UnpackOptions): Serialize[Json] = new Serialize[Json]{
    ...
    def unpack(value: MessagePack): Attempt[Json] = {
      ...
      value match {
        ...
        case MExtension8(_, code, data) => options.extension(code, data)
        case MExtension16(_, code, data) => options.extension(code, data)
        case MExtension32(_, code, data) => options.extension(code, data)
